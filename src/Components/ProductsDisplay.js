...
function ProductsDisplay(props) {
  ...
  return (
    <div className="ProductsDisplayContainer">
      ...
      <div
        className="Cart"
        style={{ visibility: props.cartVisibility ? "visible" : "hidden" }}
      >
        <div className="items">
          <h1>Your Cart: <span>{props.numberItems} Items</span></h1>
          {props.CartDetails.map((order) => {
            return (
              <div className="orderCard" key={order.id}>
                <div className="image">
                  <img src={order.images[0].url}></img>
                </div>
                <div className="details">
                  <div className="order title">{order.name}</div>
                  <div className="order cartquantity">Quantity: {order.quantity}</div>
        ...
        <div className="order total">
          <div>
            Your Total : ₹
            {props.CartDetails.reduce((total, order) => {
